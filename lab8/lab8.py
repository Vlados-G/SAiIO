...
class TaskAssignment:
    ...
    # Инициализация класса, обязательными входными параметрами являются матрица задач и метод распределения,
    # среди которых есть два метода распределения, метод all_permutation или метод Венгрии.
        ...
        while line_count < len(b):
            ...
            for i in range(len(b[0]), 0, -1):
                ...
                while (i in col_zero_count):
                    ...
                    # Нарисуйте линию, покрывающую 0, и получите матрицу после строки минус минимальное значение и
                    # столбец плюс минимальное значение
            delete_count_of_row = []
            delete_count_of_rol = []
            ...
            for i in range(len(line_order)):
                ...
                if line_count == len(b):
                    break
                    # Определяем, нужно ли рисовать линию, чтобы покрыть все нули, если она покрывает, операции
